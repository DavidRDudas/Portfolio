...
(function () {
    ...
    let lastShares = null;
    let otpResult = null;
    let fingerprintToken = 0;
    let crackStopRequested = false;
    ...
    function renderKeyReadout() {
        ...
        // Fingerprints are async; a stale one is worse than none, so tag each
        // request and drop any result that lands after a newer edit.
        const token = ++fingerprintToken;
        V.patternFingerprint(pattern).then(function (fp) {
            if (token !== fingerprintToken) return;
            const text = fp || '—';
            if ($('keyFingerprint')) $('keyFingerprint').textContent = text;
            if ($('encryptFingerprint')) $('encryptFingerprint').textContent = text;
        });
    ...
    const DEMO_ITERATIONS = 100000;   // V.MIN_ITERATIONS -- fast enough to watch

    async function generateLegacyTarget() {
        if (typeof window.encodeText !== 'function' || !window.visualCipher) {
            setStatus('attackLegacyStatus', 'The legacy cipher is not loaded on this page.', 'error');
            return;
        }
        // The legacy grid fills up from its animation timer, which is paused
        // while its tab is hidden -- so drive the tick directly rather than
        // waiting on a clock that is deliberately not running.
        const anim = window.visualCipher._legacyAnim;
        if (anim && typeof anim.fn === 'function') {
            for (let i = 0; i < 200 && window.visualCipher.activeNodes.size < 12; i++) anim.fn();
        }
        if (window.visualCipher.activeNodes.size < 3) {
            setStatus('attackLegacyStatus',
                'The legacy grid has not populated. Open the Legacy Encode tab for a moment, then retry.', 'error');
            return;
        }
        $('inputText').value = 'Rendezvous at the bridge, 23:00. Burn this.';
        await window.encodeText();
        $('attackLegacyInput').value = $('encodedOutput').value;
        setStatus('attackLegacyStatus',
            'Target generated by the legacy cipher, exactly as a user would produce it. Now break it.', 'ok');
    }

    async function runLegacyAttack() {
        const text = ($('attackLegacyInput') || {}).value || '';
        if (!text.trim()) {
            setStatus('attackLegacyStatus', 'Paste a legacy envelope, or generate one.', 'error');
            return;
        }
        $('attackLegacyResult').hidden = true;
        setStatus('attackLegacyStatus', 'Reconstructing the key schedule from the envelope...', 'busy');
        try {
            const result = await window.AttackLab.breakLegacy(text);
            $('attackPlain').textContent = result.plaintext;
            $('attackKeyBytes').textContent = result.keyBytes;
            $('attackPatternPreview').textContent = result.patternPreview +
                (result.patternPreview.length >= 160 ? '...' : '');
            $('attackIterations').textContent = result.iterations
                ? result.iterations.toLocaleString() + ' (bought nothing)'
                : 'none needed';
            $('attackHow').textContent = result.how;
            $('attackLegacyResult').hidden = false;
            setStatus('attackLegacyStatus',
                'Broken — ' + result.variant + '. No key, no password, no shared state: the envelope was enough.',
                'ok');
            toast('Legacy cipher broken');
        } catch (e) {
            setStatus('attackLegacyStatus', describeError(e), 'error');
        }
    }

    function drawWeakKey() {
        if (pattern.gridSize !== 16) {
            pattern = { gridSize: 16, steps: [] };
            if ($('keyGridSize')) $('keyGridSize').value = '16';
            buildKeyGrid();
        }
        // A short horizontal run on the default colour: the single most likely
        // thing a person draws, and squarely inside the attack dictionary.
        pattern.steps = [18, 19, 20, 21, 22].map(function (cell) {
            return { cell: cell, color: 0, rotation: 0 };
        });
        renderKeyGrid();
        setStatus('attackCrackStatus',
            'Drew a 5-cell horizontal run — the kind of key people actually pick. Now attack it.', 'ok');
    }

    async function runCrackAttack() {
        if (pattern.steps.length === 0) {
            setStatus('attackCrackStatus', 'Draw a key on the Visual Key tab first.', 'error');
            return;
        }
        const dictSize = parseInt($('attackDictSize').value, 10);
        const sentinel = 'canary::' + pattern.steps.length + ' steps';

        crackStopRequested = false;
        $('attackCrackStop').hidden = false;
        $('attackCrackRun').disabled = true;
        $('attackCrackResult').hidden = true;
        setStatus('attackCrackStatus', 'Building target envelope under your current key...', 'busy');

        try {
            const envelope = await V.encrypt(sentinel, pattern, { iterations: DEMO_ITERATIONS });
            const candidates = window.AttackLab.generateHumanPatterns(pattern.gridSize, dictSize);

            setStatus('attackCrackStatus',
                'Trying ' + candidates.length.toLocaleString() + ' human-shaped patterns...', 'busy');

            const result = await window.AttackLab.crackVisualKey(envelope, candidates, {
                shouldStop: function () { return crackStopRequested; },
                onProgress: function (p) {
                    const rate = p.tried / (p.elapsedMs / 1000);
                    $('attackProgressBar').style.width =
                        Math.min(100, (p.tried / candidates.length) * 100) + '%';
                    setStatus('attackCrackStatus',
                        'Tried ' + p.tried.toLocaleString() + ' of ' + candidates.length.toLocaleString() +
                        ' — ' + rate.toFixed(0) + ' keys/sec — ' + (p.elapsedMs / 1000).toFixed(0) + 's elapsed',
                        'busy');
                }
            });

            const seconds = Math.max(0.001, result.elapsedMs / 1000);
            const demoRate = result.tried / seconds;
            // Scale the measured rate back to the user's real iteration setting.
            const realRate = demoRate * (DEMO_ITERATIONS / currentIterations());

            if (result.found) {
                $('attackProgressBar').style.width = '100%';
                $('attackVerdict').textContent =
                    'Cracked in ' + result.tried.toLocaleString() + ' guesses (' + seconds.toFixed(1) +
                    's). Recovered plaintext: "' + result.plaintext + '". Your pattern was in a dictionary of ' +
                    'the shapes people draw — at ' + currentIterations().toLocaleString() +
                    ' iterations that is still only ' +
                    window.AttackLab.formatDuration(result.tried / realRate) + ' of work for an attacker.';
                setStatus('attackCrackStatus', 'Key recovered. This is what a weak pattern costs.', 'error');
                toast('Your visual key was cracked');
            } else {
                const bits = V.realisticEntropyBits(pattern);
                const projection = window.AttackLab.projectSearchTime(bits, realRate);
                $('attackVerdict').textContent =
                    (result.stopped ? 'Stopped after ' : 'Exhausted the dictionary after ') +
                    result.tried.toLocaleString() + ' guesses without a hit. At the measured ' +
                    realRate.toFixed(0) + ' keys/sec against ' + currentIterations().toLocaleString() +
                    ' iterations, brute-forcing your pattern\'s ' + bits.toFixed(0) +
                    ' bits would take roughly ' + projection +
                    ' on this one browser thread. A GPU cluster cuts that by orders of magnitude — ' +
                    'so treat it as a ceiling, not a guarantee.';
                setStatus('attackCrackStatus',
                    result.stopped ? 'Attack stopped.' : 'Dictionary exhausted — your pattern is not an obvious shape.',
                    'ok');
            }
            $('attackCrackResult').hidden = false;
        } catch (e) {
            setStatus('attackCrackStatus', describeError(e), 'error');
        } finally {
            $('attackCrackStop').hidden = true;
            $('attackCrackRun').disabled = false;
        }
    }
     ...
     *
     * VisualGridCipher.startGridAnimation() opens a 250ms setInterval and
     * never stores the handle, so it keeps restyling 1,025 cells forever even
     * with its tab hidden. Rather than rewrite the minified file, wrap the
     * prototype method before script.js constructs its instance and capture
     * the timer by borrowing setInterval for the duration of the call. That
     * leaves the original animation logic untouched.
     * --------------------------------------------------------------------- */

    function tameLegacyAnimation() {
        if (typeof VisualGridCipher === 'undefined') return;
        const original = VisualGridCipher.prototype.startGridAnimation;
        if (typeof original !== 'function') return;

        VisualGridCipher.prototype.startGridAnimation = function () {
            // The constructor calls initializeGrid(), which calls this; then
            // script.js calls initializeGrid() a second time, and
            // applyGridSettings() calls it again on every click. Each call
            // opened a fresh 250ms interval that nothing ever cleared, so the
            // animation silently sped up for the rest of the session. Retire
            // the previous timer before starting the next.
            if (this._legacyAnim && this._legacyAnim.timer !== null) {
                clearInterval(this._legacyAnim.timer);
                this._legacyAnim.timer = null;
            }

            const realSetInterval = window.setInterval;
            const captured = { fn: null, ms: 250, timer: null };
            window.setInterval = function (fn, ms) {
                captured.fn = fn;
                captured.ms = ms;
                captured.timer = realSetInterval.call(window, fn, ms);
            ...
            this._legacyAnim = captured;
        };
    }

    function updateLegacyAnimation() {
        const cipher = window.visualCipher;
        if (!cipher || !cipher._legacyAnim || !cipher._legacyAnim.fn) return;
        const anim = cipher._legacyAnim;
        const pane = $('encode');
        const wanted = !document.hidden && pane && pane.classList.contains('active');
        if (wanted && anim.timer === null) {
            // Because the timer no longer runs while the tab is hidden, the
            // grid would sit visibly empty for several seconds each time it is
            // opened. Run the tick directly to bring it up to a normal
            // population first, then hand back to the interval.
            for (let i = 0; i < 80 && cipher.activeNodes.size < 12; i++) anim.fn();
            anim.timer = setInterval(anim.fn, anim.ms);
        } else if (!wanted && anim.timer !== null) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Must happen before script.js's own DOMContentLoaded handler builds
        // its cipher. This file's <script> tag comes first, so its listener
        // runs first -- that ordering is what makes the wrap possible.
        tameLegacyAnimation();
        ...
        wire('attackLegacyRun', 'click', runLegacyAttack);
        wire('attackLegacySample', 'click', generateLegacyTarget);
        wire('attackCrackRun', 'click', runCrackAttack);
        wire('attackCrackWeak', 'click', drawWeakKey);
        wire('attackCrackStop', 'click', function () { crackStopRequested = true; });

        // Pause the legacy animation whenever its tab is not the visible one.
        Array.prototype.forEach.call(document.querySelectorAll('.tab-btn'), function (button) {
            button.addEventListener('click', function () { setTimeout(updateLegacyAnimation, 0); });
        });
        document.addEventListener('visibilitychange', updateLegacyAnimation);
        // Runs after script.js's DOMContentLoaded handler has built the cipher.
        setTimeout(updateLegacyAnimation, 0);
